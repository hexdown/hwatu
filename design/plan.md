...
The genesis and delta design sessions landed 2026-07-20 ([genesis.md](../../spec/design/genesis.md), [delta-schemas.md](../../spec/design/delta-schemas.md)), and their scaffold arrived ahead of the promotion: `hwatu/deltas.py` (TILL and FLUSH as library schemas — coded, not staked — each sealing to a 96-byte card with its bloom pinned in test_deltas) and the ring layout in `layouts.py` (ten petals, 6+4, with `halves` for work and `pair` for display). 67 tests green. Next: the hand-coded mary frances genesis, checked against this scaffold; then the data-model.md and store.md promotion rewrite.
