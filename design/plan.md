...
**Phase 1 is complete** (validator landed 2026-07-20; 56 tests green). The complete pipeline runs: canonical bytes → sips → tree → rendered markdown, verified end-to-end in test_seeds — and faces are now judged against their schemas with per-subtree verdicts. Phase 2 (store + orchard) is promoted to concrete items (design session 2026-08-01) and ready to implement.
